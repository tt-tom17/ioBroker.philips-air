...
const { STATE_MAPPING, detectGeneration, buildControlPayload } = require('./mapping');
...
class AirPurifier extends EventEmitter {
            ...
            this._statusRequest.on('response', res => {
                ...
                res.on('data', chunk => {
                    ...
                    try {
                        ...
                        // A single undecryptable packet (corrupt digest, unexpected format) is not a
                        // connection failure - warn and keep the observe stream open instead of erroring.
                        this.emit(
                            'warn',
                            `Ignoring undecryptable status packet from ${this.deviceIp}: ${err && err.message ? err.message : err}`,
                        );
                        return;
                    }
                    decryptPromise
                        .then(status => {
                            // Raise the decrypted status to info level when "Show device status" is enabled,
                            // otherwise keep it as a normal debug message.
                            const statusMsg = `Received status: ${status}`;
                            this.emit(this.adapter.config.showStatus ? 'info' : 'debug', statusMsg);
                            try {
                                status = JSON.parse(status);
                                this.detectScheme(status);
                            } catch {
                        ...
                        .catch(err => {
                            // Belt-and-suspenders: decryptPayload throws synchronously today, but guard the
                            // async chain too so a rejected decrypt can never become an unhandled rejection.
                            this.emit(
                                'warn',
                                `Ignoring undecryptable status packet from ${this.deviceIp}: ${err && err.message ? err.message : err}`,
                            );
    ...
    decryptPayload(encryptedData) {
        ...
        // Some firmwares pad the payload with trailing NUL bytes; strip them so the digest slice and
        // length checks below operate on the real hex string. Reject anything that is not the expected
        // hex format (counter + body + 64-char digest = at least 72 chars) with a clear error instead
        // of feeding garbage into the cipher.
        ...
        // Slice the digest off the (NUL-stripped) string length, not the raw buffer length - a buffer
        // with trailing NULs would otherwise shift the slice and never match.
        const digest = encryptedDataStr.substring(encryptedDataStr.length - 64);
        if (digest !== hash) {
            // The caller logs an ignorable-packet warning; keep the technical detail at debug level so
            // a single corrupt packet does not raise a red error.
        ...
        const payload = {
            state: {
                desired: {
                    ...
                    DeviceId: '',
                    EnduserId: '',
                    ...buildControlPayload(settings, this.generation === 'new-gen'),
                },
            },
        };
    ...
    /**
     * Detect (and remember) the device's protocol generation from the raw reported keys, so control()
     * later targets the matching scheme (old codes vs new D-codes). Sticky: an indeterminate frame
     * leaves the previous decision untouched. The raw status is left unchanged - decoding happens
     * generation-agnostically in main.js via mapReported().
     *
     * @param status the parsed device status object
     */
    detectScheme(status) {
        if (status && status.state) {
            const generation = detectGeneration(status.state.reported);
            if (generation) {
