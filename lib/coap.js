...
const { STATE_MAPPING, isNewGen, renameReported, buildControlPayload } = require('./mapping');
...
class AirPurifier extends EventEmitter {
        ...
        const payload = {
            state: {
                desired: {
                    ...
                    DeviceId: '',
                    EnduserId: '',
                    ...buildControlPayload(settings, this.newGen),
                },
            },
        };
    ...
    renameAttributes(status) {
        if (status && status.state) {
            // Detect the device generation from the raw keys before they are renamed, so control()
            // later targets the matching scheme (old codes vs new D-codes).
            this.newGen = isNewGen(status.state.reported);
